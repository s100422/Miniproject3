...
- **`metrics.price_change_12m`은 점수에 안 들어가는 참고 지표다.** 밸류에이션 축이 이미 주가를
  반대 방향으로 반영해서(오르면 비싸져서 감점), 주가 수익률 축을 더하면 둘이 상쇄돼 총점이
  아무 말도 안 하게 된다. 배당함정 판정의 입력이자 화면 표시용으로만 저장한다.
